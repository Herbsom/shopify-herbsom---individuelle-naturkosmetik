...
describe("commerce.customerAccount", () => {
  it("returns the Shopify-hosted customer account URL", async () => {
    const caller = appRouter.createCaller(makeCtx());

    await expect(caller.commerce.customerAccount()).resolves.toEqual({
      url: "https://test.myshopify.com/account",
    });
...
describe("commerce.cart", () => {
  it("creates a cart, normalizes lines, and appends channel=online_store to the checkout URL", async () => {
            ...
            edges: [
              {
                node: {
                  ...
                  quantity: 2,
                  attributes: [{ key: "Wirkstoffe", value: "Hyaluronsäure" }],
    ...
    expect(String((init as RequestInit).body)).toContain("Hyaluronsäure");
  });
