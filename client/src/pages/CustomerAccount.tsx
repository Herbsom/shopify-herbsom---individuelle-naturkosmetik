...
export default function CustomerAccount() {
  ...
  const customerAccountTarget = useMemo(() => {
    try {
      ...
      <main className="container py-20 md:py-28">
        {!data?.connected && !isLoading && (
          <section className="mx-auto max-w-3xl border border-[#E5E0D8] bg-white px-7 py-12 text-center md:px-16 md:py-18">
            ...
            <form action="/api/shopify/customer-account/login" method="get" target={customerAccountTarget} className="mt-9">
              <button type="submit" className="inline-flex items-center gap-2 bg-[#5B5B38] px-7 py-4 font-body text-xs uppercase tracking-[0.14em] text-[#F8F5F0] transition-all hover:bg-[#424226] active:scale-[0.97]">
            ...
            <a href="https://herbsomshop-az5ntglf.manus.space/api/shopify/customer-account/login" target="_blank" rel="noopener noreferrer" className="mt-3 inline-block font-body text-xs uppercase tracking-[0.1em] text-[#5B5B38] underline underline-offset-4">Shopify-Anmeldung in neuem Tab öffnen</a>
          </section>
        )}
